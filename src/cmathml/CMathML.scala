...
import cmathml.Apply.{Extractor, Extractor2}
import cmathml.CSymbol.Id
...
sealed trait CMathML extends Product {
  ...
  if (attributes.isEmpty)
    ...
    sb += '{'
    var first = true
    for (((cd,name),value) <- attributes) {
      if (!first) sb ++= ", "
      sb ++= cd; sb += '.'; sb ++= name
      sb ++= " -> "
      value.asInstanceOf[CMathML].toPopcorn(sb,0)
      first = false
    }
    sb += '}'
  }
  ...
  @Pure def updateAttributes(newAttribs : ((String,String),Any)*) : CMathML =
...
object CMathML {
  ...
  object arith1 {
    ...
    val divideE = new Extractor2(divide)
...
sealed trait Leaf extends CMathML {
...
object Apply {
  ...
  class Extractor2(cd:String, name:String) {
    def this(head:CSymbol) = this(head.cd,head.name)
    def unapply(apply:Apply) : Option[(CMathML,CMathML)] = apply match {
      case Apply(_, CSymbol(_,`cd`,`name`), a1, a2) => Some((a1,a2))
      case _ => None
    }
  }
