...
object PopcornGrammar {
  ...
  lazy val compExpr : P = P( NoCut(call) | NoCut(ecall) | NoCut(attribution) | NoCut(binding) | listExpr | setExpr | anchor ) // TODO: this does unnecessary backtracking for ecall, attribution, binding, anchor
  ...
  lazy val attribution : P = P ( anchor ~ "{" ~/ attributionList ~ "}" ).map
    { case (m,attr) => m.updateAttributes(attr:_*) }
